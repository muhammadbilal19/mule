...
public class DefaultMuleMessageBuilderTestCase extends AbstractMuleTestCase
{
    ...
    public void createAPIMessageViaMessageInterfaceFromCopy()
    {
        ...
        messageCopy = org.mule.runtime.api.message.MuleMessage.builder(message).payload(true).dataType(BOOLEAN)
    ...
    {
        MuleMessage<String, Apple> message = MuleMessage.builder().payload(TEST_PAYLOAD).dataType(STRING).attributes
                (TEST_ATTR).build();
        assertThat(message, instanceOf(DefaultMuleMessage.class));
        assertThat(message.getPayload(), is(TEST_PAYLOAD));
        assertThat(message.getDataType(), is(STRING));
    ...
    public void createMessageViaMessageInterfaceFromCopy()
    {
        ...
        assertThat(messageCopy, instanceOf(DefaultMuleMessage.class));
        assertThat(messageCopy.getPayload(), is(true));
        assertThat(messageCopy.getDataType(), is(BOOLEAN));
        assertThat(messageCopy.getAttributes(), is(TEST_ATTR_2));
    }
    ...
    {
        MuleMessage message = new DefaultMuleMessageBuilder().payload(TEST_PAYLOAD).addInboundProperty(PROPERTY_KEY, PROPERTY_VALUE, HTML_STRING).build();
        assertThat(message.getInboundProperty(PROPERTY_KEY), equalTo(PROPERTY_VALUE));
    ...
    {
        MuleMessage message = new DefaultMuleMessageBuilder().payload(TEST_PAYLOAD).addOutboundProperty(PROPERTY_KEY, PROPERTY_VALUE, HTML_STRING).build();
        assertThat(message.getOutboundProperty(PROPERTY_KEY), equalTo(PROPERTY_VALUE));
    ...
    public void mutatePayloadDifferentTypeWithMediaTypeUpdateTypeAndConserveMimeType()
    {
        ...
        MuleMessage<Boolean, Apple> copy = new DefaultMuleMessageBuilder(createTestMessage()).payload(dataHandler).build();
    ...
    {
        return new DefaultMuleMessageBuilder().payload(TEST_PAYLOAD).dataType(TEXT_STRING).attributes(TEST_ATTR).rootId
                ("1").correlationId("2").correlationGroupSize(3).correlationSequence(4).replyTo(REPLY_TO).build();
    }
